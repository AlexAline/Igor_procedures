...
function do2d(idstr1,start1,stop1,numdivs1,delay1,idstr2, start2,stop2,numdivs2,delay2)
	...
	string/G last_scan
	sprintf last_scan, "do2d(\"%s\",%f,%f,%d,%f,\"%s\",%f,%f,%d,%f)", idstr1,start1,stop1,numdivs1,delay1, idstr2,start2,stop2,numdivs2,delay2
	...
	for(i=0;i<number_of_measurements;i+=1)
		if(cmpstr(meas_type(i),"-",2)==1)
			m_name = meas_name[i]+meas_type[i]+"_"+idstr1+"_"+idstr2+"_"+num2str(nextwave())
		elseif(cmpstr(meas_type(i),"-",2)==0)
			m_name = meas_name[i]+"_"+idstr1+"_"+idstr2+"_"+num2str(nextwave())
		endif
