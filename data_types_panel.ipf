...
Window Data_Types_panel() : Panel
	...
	NewPanel /W=(391.2,57.6,916,100) as "Data Types"
	ModifyPanel cbRGB=(24576,24576,65280)
	SetDrawLayer UserBack
	SetDrawEnv fillfgc= (24576,24576,65280)
	SetDrawEnv save
	
	SetVariable number_of_measurements,pos={15,12},size={190,14},proc=set_number_of_measurements,title="Number of measurements:"
...
function create_Data_Types_panel()
	...
	for(i=0;i<max_number_of_measurements;i+=1)
		...
		PopupMenu $("device"+num2str(i)),pos={110,35+20*i},size={90,14},bodyWidth=90,proc=set_Device,title="device "
		PopupMenu $("device"+num2str(i)),value=get_linst_GPIB_devices(), disable=1, mode = meas_device_ID[i]+1

		PopupMenu $("datatype"+num2str(i)),pos={205,35+20*i},size={70,14},bodyWidth=70,proc=set_MeasType,title=" "
		PopupMenu $("datatype"+num2str(i)),value="", disable=1
	
		SetVariable $("calc_"+num2str(i)), value=calc_eq[i], title="M["+num2str(i)+"]=", pos={280,35+i*20}, size={120,14}, disable=1
		
		CheckBox $("log_disp"+num2str(i)), pos={410,35+20*i},size={80,14},bodyWidth=70,proc=set_meas_log,title="log "
		CheckBox $("log_disp"+num2str(i)), side=1, disable=1, value=meas_log[i]
	endfor
end

function/T get_linst_GPIB_devices()
...
//	print ctrlName,varNum,varStr,varName
	...
	movewindow V_left, V_top, V_right, V_top+35+15*number_of_measurements
end
