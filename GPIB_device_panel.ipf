...
Window GPIB_Devices_panel() : Panel
	...
	NewPanel /W=(1230,40,1570,80) as "GPIB Devices"
	ModifyPanel cbRGB=(24576,24576,65300)
	SetVariable number_of_devices,pos={8,12},size={150,14},proc=set_number_of_devices,title="Number of devices:"
	SetVariable number_of_devices,limits={0,max_number_of_devices,1},value= number_of_devices
	Button init_button, pos={170,12}, size={70,18}, proc=init_devices, title="Connect"
	Button deinit_button, pos={250,12}, size={70,18}, proc=deinit_devices, title="Disconnect"
...
function create_GPIB_Devices_panel()
	...
	for(i=1;i<max_number_of_devices+1;i+=1)
		SetVariable $("ctr_Name_"+num2str(i)), value=GPIB_names[i], title="Name: ", size={130,14}, pos={15,15+i*20}, disable=1, proc=name_was_set
		TitleBox $("dev_family_"+num2str(i)), size={60,14}, pos={148,15+i*20}, disable=1, frame=0
		SetVariable $("ctr_Address_"+num2str(i)), value=GPIB_addresses[i], size={66.00,14}, pos={195.00,15+i*20}, disable=1,title="GPIB",limits={0,31,1}
		TitleBox $("instr_ID_"+num2str(i)), size={60,14}, pos={270,15+i*20}, disable=1, frame=0
	endfor
end
...
function set_number_of_devices(ctrlName,varNum,varStr,varName) : SetVariableControl
	...
	movewindow V_left, V_top, V_right, V_top+35+15*varNum
end
...
function init_device_meas_table()
	...
	device_meas_table[i][1] = "-;loM;loPh;"
	i+=1
